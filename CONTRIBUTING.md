...
카탈로그 항목 소개(`registry/integrations.json`의 `description`), 공지·안내
문구처럼 플레이어나 외부 사용자에게 노출되는 글은 **별도 지시가 없는 한 무조건
친절한 해요체**로 쓴다(합쇼체·반말 금지). 개발자 대상 기술 문서(README, API
레퍼런스 등)는 이 규칙의 대상이 아니다.
